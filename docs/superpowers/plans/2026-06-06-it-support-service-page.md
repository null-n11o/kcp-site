...
- [ ] **Step 0: feature ブランチを作成する**

```bash
git checkout -b feat/it-support-service-page
```

- [ ] **Step 1: `hero.subtext` を更新する**
...
- [ ] **Step 2: `service.items` の先頭に丸投げIT担当を追加する**
...
- [ ] **Step 1: フロントマターに `ticketGuide` データを追加する**
...
const ticketGuide = [
  ...
  { task: 'X・Threads投稿1本 / Instagram リール・TikTok動画（1分）', tickets: '1' },
  { task: '文字改修など軽微なシステム改修・設定変更', tickets: '1' },
  { task: 'LP・資料・PDF作成（シンプルなもの）', tickets: '2' },
  { task: 'LP・資料・PDF作成（複数ページ・デザイン込み）', tickets: '3' },
  { task: 'AI・自動化対応', tickets: '3（要件に合わせて変動）' },
  { task: 'AI導入支援・設定代行', tickets: '要件に合わせて事前にお見積り' },
  { task: '新規システム開発', tickets: '要件に合わせて事前にお見積り' },
];
```

- [ ] **Step 2: PLAN + TICKET GUIDE セクションを `</PageLayout>` の直前に追加する**
...
- [ ] **Step 1: フロントマターに `scopeItems` と `comparisonRows` を追加する**
...
- [ ] **Step 2: SCOPE + COMPARISON セクションを `</PageLayout>` の直前に追加する**
...
- [ ] **Step 1: フロントマターに `fitItems` と `faqs` を追加する**
...
- [ ] **Step 2: FIT FOR + FAQ + CTA セクションを `</PageLayout>` の直前に追加する**
...
- [ ] **Step 3: プランファイルの完了ステップを更新してコミット**
...
## Test plan
- [ ] \`/services/it-support/\` — 9セクション（Hero〜CTA）が全て表示される
- [ ] FAQ アコーディオンが JS なしで開閉する
- [ ] トップページのサービス一覧先頭に「丸投げIT担当」が表示される
- [ ] トップページのヒーローサブテキストが更新されている
