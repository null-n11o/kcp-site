...
2. 「質問」「回答」「設定」のタブのうち「設定」タブを開き、「回答」（回答を収集、保護する方法を管理できます）を展開して、「メールアドレスを収集する」を **ON** に設定（必須。これがないと Apps Script が送信先アドレスを取得できない）
3. 質問を追加（いずれも任意回答で可）:
   - 「会社名」（記述式）
   - 「お名前」（記述式）
4. 「設定」タブ →「表示設定」（フォームと回答の表示方法を管理できます）を展開 →「確認メッセージ」の項目を編集し、以下を設定:
